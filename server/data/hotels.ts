...
export const HOTELS: Hotel[] = [
  ...
  },
  {
    ...
    category: "Boutique",
    featured: false,
    amenities: ["Wi-Fi", "Piscina", "Biblioteca", "Restaurante", "Bar", "Spa"],
    policies: {
      checkIn: "14:00",
      checkOut: "12:00",
      cancellation: "Cancelamento gratuito até 48h antes",
...
export type ListParams = {
  ...
  page?: number;
  limit?: number;
  projection?: "full" | "list"; // full = objeto completo (detalhe), list = resumo
};
...
export function listHotels(params: ListParams = {}) {
  ...
  if (ids?.length) {
    data = data.filter((h) => ids.includes(h.id));
  }
  ...
  if (destination) {
    ...
    data = data.filter(
      (h) =>
        h.location.toLowerCase().includes(d) || h.name.toLowerCase().includes(d)
    );
  }

  if (typeof priceMin === "number") {
    data = data.filter((h) => h.pricePerNight >= priceMin);
  }
  ...
  // Projeção mais leve para listagem (opcional)
  const sliced = data.slice(start, end).map((h) => {
    if (projection === "full") return h;
    return {
      id: h.id,
      name: h.name,
      location: h.location,
      images: h.images?.slice(0, 1), // thumbnail
      pricePerNight: h.pricePerNight,
      rating: h.rating,
      reviewCount: h.reviewCount,
      category: h.category,
      featured: h.featured,
    } satisfies Partial<Hotel>;
  });
...
export function findHotelById(id: string) {
